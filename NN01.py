''' 
    ...
    based on Sharpe ratio of returns of the last sharpe_datframe and 
    based on Hurst exponent of prices of the last hurst_datframe.
...
def hurst(price):
    '''
        Calculate Hurst Exponent based on len(price)
    '''
    # Initiate tau and lag vectors
    tau = []; lagvec = []
    # Step through the different lag values
    for lag in range(2, hurst_trainframe): # if len(price) is too big, this can be reduced for the artificial series
...
def hurst_r(context, data):
    ...
    if len(context.past_prices) < hurst_datframe: # len: return the length (the number of items) of an object
        return
    # Initiate tau and lag vectors
    tau, lagvec = [], []
    # Step through the different lag values
    for lag in range(2, hurst_datframe): #include all datasets as chosen
        # Calculate the price difference with lag
...
def sharpe(series):
    ...
    #sharpe = sharpe*numpy.sqrt(sharpe_periods_per_year/float(sharpe_trainframe))
...
def sharpe_r(context, data):
    ...
    if len(context.past_prices) < sharpe_datframe:
        return
    ...
    sharpe = mean/std
    # Sharpe * sqrt(number of periods in a year)
    #sharpe = sharpe*numpy.sqrt(sharpe_periods_per_year/float(sharpe_datframe))
    sharpe = sharpe*numpy.sqrt(sharpe_periods_per_year)
    return sharpe
...
def rnd_teach():
    ...
    if k == 0: # if d>0: Mean Reverting data / Hurst<0.5
        d = numpy.random.random()*0.40 + 0.05 # average is 0.25
        while(hurst_ret >= 0.5):
            dummy, dummy, F = simulate_cointegration(d, sharpe_trainframe, 0, 0.5, 0.0, 0.0)
            sharpe_ret = sharpe(F[1:])
            hurst_ret = hurst(F[1:])
    elif k == 1: # if offset=0: Momentum data / Hurst>0.5 
        offset = numpy.random.random()*0.10 + 0.05 # average is 0.15
        while(hurst_ret <= 0.5):
            F = simulate_momentum_data(sharpe_trainframe, offset, 0.9)
            sharpe_ret = sharpe(F[1:])
            hurst_ret = hurst(F[1:])
    elif k == 2: # Random walk data, since offset=0
        F = simulate_momentum_data(sharpe_trainframe, 0, 0.9)
        sharpe_ret = sharpe(F[1:])
        hurst_ret = hurst(F[1:])
...
def gather_prices(context, data, sid):
    ''' 
        Get context.past_prices in data frame 'sharpe_datframe'
    '''
...
def myhandle_data(context,data):
    ...
    # Get context.past_prices up to 'sharpe_datframe' and increase len(context.past_prices)
    ...
    if (context.periods_traded <= NN_trainframe):
        ...
        while(count < NN_count):
            ...
            if (context.periods_traded == NN_trainframe and count == NN_count):
    ...
    elif (context.periods_traded > NN_trainframe and len(context.past_prices) < sharpe_datframe):
        log.debug('periods_traded > NN_trainframe and len(prices) < sharpe_datframe')

    # Ordering phase after initial calibration starts, if there are enough prices to calculate Sharpe and Hurst
    elif (len(context.past_prices) >= sharpe_datframe and len(context.past_prices) >= hurst_datframe):
            ...
            momentum_old = context.momentum
            reverting_old = context.reverting
            context.momentum = False
            context.reverting = False
            if (f_output > 0.75 and h_output > 0.5):
                ...
                if momentum_old != context.momentum:
                    log.debug('Momentum change from %s to %s' % momentum_old, context.momentum)
                    log.debug('Price:       %g' % data.current(context.security, 'price'))
                    log.debug('Mean:        %g' % mov_avg_price)
                    log.debug('NN:          %s' % f_output)
                    log.debug('Hurst:       %s' % h_output)
                ...
                if momentum_old != context.momentum:
                    log.debug('Reverting change from %s to %s' % reverting_old, context.reverting)
                    log.debug('Price:       %g' % data.current(context.security, 'price'))
                    log.debug('Mean:        %g' % mov_avg_price)
                    log.debug('NN:          %s' % f_output)
                    log.debug('Hurst:       %s' % h_output)
...
# Sharpe will only return valid values if the length of the list is greater than sharpe_datframe
# Set up a data frame for the data length of 'context.past_prices'
sharpe_periods_per_year = int(210 * 6.5 * 2) # 210 days of 6:30 hours in US_Equities + myhandle_minutes = 30
sharpe_datframe = int(3 * 6.5 * 2) # 3 days of 6:30 hours in US_Equities + myhandle_minutes = 30

# Hurst will only return valid values if the length of the list is greater than hurst_datframe
# hurst_datframe needs to be smaller or equal to sharpe_datframe
hurst_datframe = int(sharpe_datframe*0.80)

# To calculate Sharpe and Hurst during initial training precisely, a larger dataset than sharpe_datframe can be used
sharpe_trainframe = 100
hurst_trainframe = int(sharpe_trainframe*0.80)

# Neural Network learning phase for initial calibration
NN_training_tot=2000 # total training cycles in initial NN training phase
NN_count=2000        # training cycles per period during initial NN training phase
NN_trainframe = max(1, int(NN_training_tot / NN_count)) # number of periods of initial NN training phase

# Neural Network learning rate
NN_alpha = 0.06          # learning rate
NN_alpha_decay = 0.8     # learning rate decay every 1000 steps

# Neural Network layout for feed forward network
nIn = 2         # Input is Sharpe and Hurst
nHidden = 10    # Hidden Layer
